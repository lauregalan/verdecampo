...
import { Edit2, Plus, Search, Trash2, FileSpreadsheet } from "lucide-react";
...
export default function Cosechas() {
    ...
    const [cosechaEditando, setCosechaEditando] = useState<Cosecha | null>(
        null,
    );
    const [cosechaAEliminar, setCosechaAEliminar] = useState<Cosecha | null>(
        null,
    );
    ...
    const handleCosechaSaved = (cosecha: Cosecha) => {
        if (cosechaEditando) {
            setCosechas((prev) =>
                prev.map((c) => (c.id === cosecha.id ? cosecha : c)),
            );
        } else {
    ...
    const handleEliminar = async () => {
        ...
        try {
            const response = await api.delete(
                `/api/cosechas/${cosechaAEliminar.id}`,
            );
            if (!response.ok) throw new Error();
            setCosechas((prev) =>
                prev.filter((c) => c.id !== cosechaAEliminar.id),
            );
    ...
    const columns: ColumnDef<Cosecha>[] = [
        ...
        },
        {
            ...
            headerClassName:
                "px-6 py-4 text-left text-sm font-semibold text-gray-900",
            cellClassName:
                "px-6 py-4 text-sm text-gray-900 font-semibold tabular-nums",
            cell: (c) =>
                Number(c.rinde).toLocaleString("es-AR", {
            ...
            <div className="min-h-full p-8 font-sans">
                ...
                <div className="mx-auto mb-4 max-w-[1600px]">
                    <div className="flex gap-2 items-center">
                        ...
                        <a
                            href="/cosechas/exportar/excel"
                            className="inline-flex items-center gap-2 rounded-xl bg-green-700 px-4 py-2 font-semibold text-white shadow-sm transition-all hover:bg-green-800 hover:shadow-md active:scale-95 whitespace-nowrap"
