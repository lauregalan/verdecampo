...
export default function Campania() {
    ...
    const [selectedEstado, setSelectedEstado] =
        useState<CampaignStatus>("Planificada");
    ...
    const summary = useMemo(
        () => ({
            ...
            enCurso: campanias.filter(
                (campania) => campania.estado === "En Curso",
            ).length,
            planificadas: campanias.filter(
                (campania) => campania.estado === "Planificada",
            ).length,
            fields: new Set(
                campanias
                    .map((campania) => campania.campo_id)
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const url = editingCampaniaId
            ? `/api/campanias/${editingCampaniaId}`
            : "/api/campanias";
        ...
        try {
            ...
            setFormError(
                err instanceof Error
                    ? err.message
                    : "No se pudo guardar la campania.",
            );
        } finally {
            setSaving(false);
        }
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-stone-500">
                                Administra estados, fechas y campos asociados de
                                cada temporada.
                            </p>
                        </div>
                        <button
                    ...
                    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                        {[
                            ...
                            },
                            {
                                ...
                                detail: "Con campanias asociadas",
                            },
                        ].map((item) => (
                            <article
                                key={item.label}
                            ...
                            {loading ? (
                                ...
                                filteredCampanias.map((campaign) => (
                                    <CampaignCard
                                        ...
                                        onEdit={() =>
                                            void openEditModal(campaign.id)
                                        }
                                    />
                                ))
                            )}
                ...
                <div className="p-6">
                    ...
                    {detailCampania ? (
                        <div className="space-y-3">
                            <p>
                                <strong>Nombre:</strong> {detailCampania.nombre}
                            </p>
                            <p>
                                <strong>Campo:</strong>{" "}
                                {detailCampania.campo_id !== null
                                    ? (fieldById[detailCampania.campo_id] ??
                                      "Campo desconocido")
                                    : "N/A"}
                            </p>
                            <p>
                                <strong>Cultivo:</strong>{" "}
                                {detailCampania.cultivo_id !== null
                                    ? (cultivoById[detailCampania.cultivo_id] ??
                                      "Cultivo desconocido")
                                    : "N/A"}
                            </p>
                            <p>
                                <strong>Fecha inicio:</strong>{" "}
                                {formatDate(detailCampania.fecha_inicio)}
                            </p>
                            <p>
                                <strong>Fecha fin:</strong>{" "}
                                {formatDate(detailCampania.fecha_fin)}
                            </p>
                            <p>
                                <strong>Estado:</strong> {detailCampania.estado}
                            </p>
                    ...
                    <div className="space-y-4">
                        <div>
                            <InputLabel
                                ...
                                value="Nombre de la campania"
                            />
                            <TextInput
                                id="campania-nombre"
                                value={newCampaniaName}
                                onChange={(event) =>
                                    setNewCampaniaName(event.target.value)
                                }
                        ...
                        <div>
                            ...
                            <select
                                ...
                                onChange={(event) =>
                                    setSelectedCampoId(event.target.value)
                                }
                        ...
                        <div>
                            ...
                            <select
                                ...
                                onChange={(event) =>
                                    setSelectedCultivoId(event.target.value)
                                }
                                className="mt-2 w-full rounded-2xl border border-stone-300 bg-stone-50 px-4 py-3 text-sm text-stone-800 outline-none transition focus:border-emerald-500 focus:bg-white"
                            >
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            <div>
                                ...
                                <TextInput
                                    ...
                                    onChange={(event) =>
                                        setNewFechaInicio(event.target.value)
                                    }
                                    className="mt-2 w-full rounded-2xl border-stone-300 bg-stone-50"
                                    required
                                />
                            ...
                            <div>
                                ...
                                <TextInput
                                    ...
                                    value={newFechaFin}
                                    onChange={(event) =>
                                        setNewFechaFin(event.target.value)
                                    }
                                    className="mt-2 w-full rounded-2xl border-stone-300 bg-stone-50"
                                />
                            </div>
