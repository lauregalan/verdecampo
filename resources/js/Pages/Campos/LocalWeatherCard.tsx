...
export const LocalWeatherCard: React.FC<LocalWeatherCardProps> = ({ clima }) => {
    if (!clima || !clima.temperatura_max.length) {
        return (
            <Card className="flex h-full flex-col overflow-hidden rounded-xl border-stone-200 bg-white shadow-sm">
    ...
    return (
        <Card className="flex h-full min-h-0 flex-col overflow-hidden rounded-xl border-stone-200 bg-white shadow-sm">
            {/* Cuerpo: Temperatura Actual */}
            <div className="flex flex-1 flex-col items-center justify-center p-4">
                <div className="flex items-center gap-2">
                    <CurrentIcon className="size-6 text-amber-500 animate-pulse-slow" />
                ...
                <div className="mt-2 flex items-baseline">
                    <span className="text-5xl font-black tracking-tighter text-stone-900">{currentTemp}</span>
                    <span className="text-2xl font-light text-stone-300">°C</span>
                </div>
                
                <div className="mt-1 flex items-center gap-1.5 text-xs font-medium text-stone-500">
                    <Thermometer size={14} />
            ...
            <div className="grid grid-cols-3 border-t border-stone-100 bg-stone-50/50 p-2">
                {forecast.map((item, index) => (
                    <div key={index} className="flex flex-col items-center gap-0.5 border-stone-200 last:border-0 [&:not(:last-child)]:border-r">
